...
export default (props) => {
  ...
  const columnOrder = ['denie', 'filame', 'luster', 'stock', 'remark'];
  ...
  const renderRow = (row, i1) => row.types.map((typeData, i2) => (
    <tr key={`${i1}-${i2}`}>
      ...
      <td className={clsx(classes.tdth, classes.typeTd)}>
        {typeData.type}
      </td>
      {columnOrder.map((columnName) => {
        const valueTable = typeData[columnName];
        return (
          <td key={columnName} className={clsx(classes.tdth, classes.dataTd)}>
